...
#define LASER_4 10
#define LASER_5 11
#define LASER_6 26 //Need to be tested

char laser_pattern [6] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20};
...
void save_thread_func(void * lpParam){
	...
	#endif
	dummy_image = cvCreateImage(cvSize(IMG_WIDTH, IMG_HEIGHT), IPL_DEPTH_8U, 1);
	printf("Start Save ! \n");
	while(thread_alive){
...
void acq_image_thread_func(void * lpParam){
	...
	raspiCamCvFlashEnable(LASER_1);
	raspiCamCvFlashEnable(LASER_2);
	raspiCamCvFlashEnable(LASER_3);
	raspiCamCvFlashEnable(LASER_4);
	raspiCamCvFlashEnable(LASER_5);
	raspiCamCvFlashEnable(LASER_6);
	raspiCamCvSetFlashPattern(laser_pattern, PATTERN_LENGTH);
...
int main(int argc, char *argv[]){
	...
	if(init_frame_buffer(&my_frame_buffer,nb_frames, (IMG_WIDTH*IMG_HEIGHT)+sizeof(float)) < 0){
		printf("Cannot allocate %d bytes \n", (IMG_WIDTH*IMG_HEIGHT)*nb_frames);
		exit(-1);
	}
